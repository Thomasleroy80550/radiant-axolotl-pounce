...
const formSchema = z.object({
  objective: z.coerce.number()
    .min(0, { message: "L'objectif ne peut pas être inférieur à 0." })
    .max(100, { message: "L'objectif ne peut pas dépasser 100." })
...
const ObjectiveDialog: React.FC<ObjectiveDialogProps> = ({
  ...
  React.useEffect(() => {
    // Reset form with current objective when dialog opens or currentObjective changes
    if (isOpen) {
      form.reset({ objective: currentObjective });
    }
  }, [isOpen, currentObjective, form]);

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await updateProfile({ objective_percentage: values.objective });
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Définissez votre objectif de performance annuel en pourcentage.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
